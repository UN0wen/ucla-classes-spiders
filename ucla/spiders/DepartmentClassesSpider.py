    ...
    def start_requests(self):
        with open("./data/dept.json", "r") as f:
            ...
            for dept in depts:
                ...
                yield scrapy.Request(url=dept_url, callback=self.parse, headers=headers, cb_kwargs=dict(dept=dept_code, page=1, cids=[], cnames=[]))

    def parse(self, response, dept, page, cids, cnames):
        ...
        if (not class_ids) or (not class_names):
            classes = []
            for id, name in zip(cids, cnames):
                classes.append(dict(class_id=id, class_name=name))
        ...
        yield scrapy.Request(url=url, callback=self.parse, headers=headers, cb_kwargs=dict(dept=dept, page=page, cids=cids, cnames=cnames))
